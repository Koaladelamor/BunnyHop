...
namespace Q3Movement
{
    ...
    public class Quake3Movement : MonoBehaviour
    {
        ...
        private void Update()
        {
            m_MoveInput = new Vector3(Manager_Input._INPUT_MANAGER.GetLeftAxis().x, 0, Manager_Input._INPUT_MANAGER.GetLeftAxis().y);
        ...
        private void QueueJump()
        {
            if (m_AutoBunnyHop)
            {
                m_JumpQueued = Manager_Input._INPUT_MANAGER.GetJumpButtonDown();
                return;
            }

            if (Manager_Input._INPUT_MANAGER.GetJumpButtonDown() && !m_JumpQueued)
            {
                m_JumpQueued = true;
            }
